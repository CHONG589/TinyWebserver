...
#include "http/transitionFiles/http_parser.h"
...
static zch::Logger::ptr g_logger = SYLAR_LOG_NAME("system");

static zch::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    zch::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

static zch::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    zch::Config::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

static zch::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    zch::Config::Lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

static zch::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    zch::Config::Lookup("http.response.max_body_size", (uint64_t)(64 * 1024 * 1024), "http response max body size");
...
size_t HttpRequestParser::Execute(char *data, size_t len) {
    ...
    if (m_parser.upgrade) {
        //处理新协议，暂时不处理
        LOG_DEBUG(g_logger) << "found upgrade, ignore";
        SetError(HPE_UNKNOWN);
    } else if (m_parser.http_errno != 0) {
        LOG_DEBUG(g_logger) << "parse request fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
...
size_t HttpResponseParser::Execute(char *data, size_t len) {
    ...
    if (m_parser.http_errno != 0) {
        LOG_DEBUG(g_logger) << "parse response fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
        SetError((int8_t)m_parser.http_errno);
    } else {
        if (nparsed < len) {
            memmove(data, data + nparsed, (len - nparsed));
