...
/**
* @brief 取消所有事件, 所有被注册的回调事件在cancel之前都会被执行一次
* @param[in] fd 描述符
* @return true 
* @return false 
*/
...
int IOManager::setnonblocking(int fd) {
    // 返回给定文件描述符 fd 的文件状态标志。这些标志通常用于描述文件的
    // 打开方式（例如只读、只写、读写等）以及其他特性（例如非阻塞模式）
...
/**
* @brief 通知调度器有任务要调度, 写pipe让idle协程从epoll_wait
...
/**
* @brief 判断是否可以停止, 判断条件是Scheduler::stopping()
* 外加IOManager的m_pendingEventCount为0，表示没有IO事件可调度了
* @return true 
* @return false 
*/
...
* @param[out] timeout 最近一个定时器的超时时间
* @return true 
* @return false 
*/
...
/**
* @brief 获取事件上下文类，Event 是上面的的 enum 值，是十六进制
* 的值，通过传进去event，返回对应的EventContext read 或 EventContext write
* @param[in] event 
* @return EventContext& 返回的事件上下文
*/
...
/**
* @brief 触发事件, 根据事件类型调用对应上下文结构中的调度器去调度
* 回调协程或回调函数
* @param[in] event 
*/
...
/**
* @brief 对于IO协程调度来说，应阻塞在等待IO事件上，idle退出
* 的时机是epoll_wait返回，对应的操作是tickle或注册的IO事件发生
*/
void IOManager::idle() {
